...
async function main() {
    const source = readFileSync(
        join(process.cwd(), "app/lib/data/vehicles/cars/index.ts"),
        "utf8"
    );
    const importMap = parseImportMap(source);
    const vehicles = parseVehicles(source, importMap).filter((vehicle) =>
        MISSING_CAR_IDS.includes(vehicle.legacyId)
    );

    console.log(`Veicoli mancanti da importare: ${vehicles.length}`);
    ...
    for (const vehicle of vehicles) {
        ...
        if (existing) {
            console.log(`Salto ${vehicle.title} (già presente)`);
            continue;
        }

        const folder = `speedservice/vehicles/cars/${vehicle.legacyId}`;
    ...
    console.log("Import veicoli mancanti completato.");
}
